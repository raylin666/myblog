...
class Api
{
	...
	public static function friendSubmit($self)
	{
		...
		$friend = Db::name('friends')->whereLike('url', '%' . $domain . '%')->find();
		if ($friend) {
			return ['code' => 0, 'message' => ($friend['status'] ? '本站已有您的友情链接！' : '您已提交过友链，请耐心等待审核')];
		}

		$insert_id = Db::name('friends')->insertGetId([
			'title' => $title,
			'url' =>  $url,
			'logo' =>  $logo,
		...
		if (!$insert_id) return ['code' => 0, 'message' => '提交失败，请联系本站点管理员进行处理'];
		if (self::$options->JFriendEmail == 'on') {
			$EmailTitle = '友链申请';
			$subtitle = $title . ' 向您提交了友链申请';
			$panel = urlencode('../themes/' . THEME_NAME . '/admin/friends.php');
			$operate_url = self::$options->siteUrl .  'admin/extending.php?panel=' . $panel . '&id=' . $insert_id . '&action=';
			$operate = [
				'<a href="' . $operate_url . 'open">通过审核</a>',
				'<a href="' . $operate_url . 'delete">删除申请</a>'
			];
			$content = ['站点标题' => $title, '站点链接' => $url, '站点图标' => $logo, '站点描述' => $description, '对方邮箱' => $email, '快捷操作' => implode('丨', $operate)];
